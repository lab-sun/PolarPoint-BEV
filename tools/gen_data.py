...
def gen_single_route(route_folder):
	...
	for i in range(INPUT_FRAMES-1, length-FUTURE_FRAMES):
		...
		bev_digit_list = [route_folder.replace(data_path, '') + "/bev_digit/"f"{str(i - _).zfill(4)}.json" for _ in
		                  range(INPUT_FRAMES - 1, -1, -1)]
		seq_bev_digit.append(bev_digit_list)
	...
	return seq_future_x, seq_future_y, seq_future_theta, seq_future_feature, \
		seq_future_action, seq_future_action_mu, seq_future_action_sigma, \
		seq_future_only_ap_brake, seq_input_x, seq_input_y, seq_input_theta, \
		seq_front_img, seq_bev_digit,seq_feature, seq_value, seq_speed, seq_action, seq_action_mu, \
		seq_action_sigma, seq_x_target, seq_y_target, seq_target_command, seq_only_ap_brake
...
def gen_sub_folder(folder_path):
	...
	for route in route_list:
		...
		if not seq_data:
			continue
		seq_future_x, seq_future_y, seq_future_theta, seq_future_feature, \
			seq_future_action, seq_future_action_mu, seq_future_action_sigma, \
			seq_future_only_ap_brake, seq_input_x, seq_input_y, seq_input_theta, \
			seq_front_img, seq_bev_digit, seq_feature, seq_value, seq_speed, seq_action, seq_action_mu, \
			seq_action_sigma, seq_x_target, seq_y_target, seq_target_command, seq_only_ap_brake = seq_data
		...
		total_bev_digit.extend(seq_bev_digit)
	...
	data_dict['bev_digit'] = total_bev_digit
	...
	file_path = os.path.join(folder_path, "packed_data_bev")
