...
def gen_single_route(route_folder):
	...
	for i in range(INPUT_FRAMES-1, length-FUTURE_FRAMES):
		...
		graph_list = [route_folder.replace(data_path, '') + "/graph_large/"f"{str(i - _).zfill(4)}.json" for _ in
		                  range(INPUT_FRAMES - 1, -1, -1)]
		seq_graph.append(graph_list)
	...
	return seq_future_x, seq_future_y, seq_future_theta, seq_future_feature, \
		...
		seq_front_img, seq_graph,seq_feature, seq_value, seq_speed, seq_action, seq_action_mu, \
...
def gen_sub_folder(folder_path):
	...
	file_path = os.path.join(folder_path, "packed_data_large")
