...
COLOR_ALUMINIUM_5 = (255, 255, 255) # road
COLOR_BLACK = (0, 0, 0) # background black
COLOR_BLUE = (0, 0, 255) # vehicle blue
COLOR_CYAN = (0, 255, 255) # walker
COLOR_RED = (255, 0, 0) # tl_red
COLOR_Lane = (0, 255, 0) # lane green
COLOR_YELLOW = (255, 255, 0) # tl_yellow

COLOR_WHITE = COLOR_ALUMINIUM_5 # ego-vehicle
COLOR_GREEN = COLOR_ALUMINIUM_5 # tl_green
COLOR_MAGENTA = COLOR_ALUMINIUM_5 # divider
COLOR_MAGENTA_2 = COLOR_ALUMINIUM_5 # divider
COLOR_YELLOW_2 = COLOR_ALUMINIUM_5 # stop mask
...
class ObsManager():
	...
	def get_observation(self, route_plan):
		...
		for i, mask in enumerate(stop_masks):
			image[mask] = tint(COLOR_YELLOW_2, (h_len-i)*1.0)
		for i, mask in enumerate(tl_green_masks):
			image[mask] = tint(COLOR_GREEN, (h_len-i)*1.0)
		for i, mask in enumerate(tl_yellow_masks):
			image[mask] = tint(COLOR_YELLOW, (h_len-i)*1.0)
		for i, mask in enumerate(tl_red_masks):
			image[mask] = tint(COLOR_RED, (h_len-i)*1.0)

		for i, mask in enumerate(vehicle_masks):
			image[mask] = tint(COLOR_BLUE, (h_len-i)*1.0)
		for i, mask in enumerate(walker_masks):
			image[mask] = tint(COLOR_CYAN, (h_len-i)*1.0)
